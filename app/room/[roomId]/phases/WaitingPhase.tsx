...
interface WaitingPhaseProps {
  ...
  currentPlayerId: string; isMaster: boolean;
}
...
export function WaitingPhase({ roomId, room, players, currentPlayerId, isMaster }: WaitingPhaseProps) {
  ...
  const { reactionEffects, isReactionPanelOpen, setIsReactionPanelOpen, reactionPanelRef, reactionToggleButtonRef, handleSendReaction } = useReactions(roomId, currentPlayerId, currentPlayer?.nickname);
  const { timeLimit, setTimeLimit, correctAnswerPoints, setCorrectAnswerPoints, fastestAnswerBonusPoints, setFastestAnswerBonusPoints, wrongAnswerPenalty, setWrongAnswerPenalty, predictionHitBonusPoints, setPredictionHitBonusPoints, isSavingOptions, handleSaveOptions } = useRoomOptions(roomId, room);
  ...
  const handleLeaveOrDisband = async () => {
    ...
    if (success) { sessionStorage.removeItem('currentPlayerId'); sessionStorage.removeItem('currentRoomId'); router.push('/'); }
    setIsLeaving(false);
  };

  const handleCopyScreenUrl = async () => {
    if (!screenUrl) return;
    try { await navigator.clipboard.writeText(screenUrl); setIsCopyingScreenUrl(true); setTimeout(() => setIsCopyingScreenUrl(false), 2000); }
    catch (error) { console.error('Failed to copy screen URL:', error); }
  };
  ...
  return (
    <>
      <div className="space-y-8">
        ...
        <div className="bg-gradient-to-br from-slate-800/50 to-slate-900/50 rounded-lg border border-slate-700/50 p-4 mb-2 space-y-3">
          ...
          <div className="pt-2 border-t border-slate-700/50 flex flex-wrap gap-3">
            {[
              ...
              <div key={opt.label} className="flex items-center gap-1.5"><span className="text-xs text-slate-400 italic">{opt.label}</span><span className={`text-xs font-semibold ${opt.color || 'text-white'}`}>{opt.value}</span></div>
            ))}
          </div>
          {room.useScreenMode && room.displayDeviceId && isMaster && (
            <div className="pt-3 border-t border-slate-700/50 space-y-2">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                ...
                <button type="button" onClick={handleCopyScreenUrl} className="bg-blue-600 hover:bg-blue-700 text-white text-xs px-3 py-2 rounded transition-all">{isCopyingScreenUrl ? 'コピー済み' : 'リンクをコピー'}</button>
              </div>
            </div>
          )}
        </div>
        ...
        {currentPlayer && (
          <ReactionTrigger isReactionPanelOpen={isReactionPanelOpen} setIsReactionPanelOpen={setIsReactionPanelOpen} reactionPanelRef={reactionPanelRef} reactionToggleButtonRef={reactionToggleButtonRef} handleSendReaction={handleSendReaction} quickMessages={WAITING_QUICK_MESSAGES} />
        )}
